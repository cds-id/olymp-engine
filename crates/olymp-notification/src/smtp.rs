...
impl SmtpProvider {
    ...
    pub fn from_env() -> Result<Self, AppError> {
        let host = dotenvy::var("OLYMP__EMAIL__SMTP_HOST")
            .unwrap_or_else(|_| "smtp.larksuite.com".to_string());
        let port: u16 = dotenvy::var("OLYMP__EMAIL__SMTP_PORT")
            .unwrap_or_else(|_| "587".to_string())
            .parse()
            .unwrap_or(587);
        let username = dotenvy::var("OLYMP__EMAIL__SMTP_USER")
            .map_err(|_| AppError::Internal("OLYMP__EMAIL__SMTP_USER not set".into()))?;
        let password = dotenvy::var("OLYMP__EMAIL__SMTP_PASSWORD")
            .map_err(|_| AppError::Internal("OLYMP__EMAIL__SMTP_PASSWORD not set".into()))?;
        let from_address = dotenvy::var("OLYMP__EMAIL__SMTP_FROM")
            .unwrap_or_else(|_| "no-reply@ciptadusa.com".to_string());
        let from_name = dotenvy::var("OLYMP__EMAIL__FROM_NAME")
            .unwrap_or_else(|_| "Olymp LMS".to_string());
