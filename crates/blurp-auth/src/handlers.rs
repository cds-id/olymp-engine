...
use blurp_core::response::{ApiResponse, WithStatus};
use blurp_notification::models::MagicLinkData;
use crate::AppState;
use crate::extractors::AuthUser;
use crate::address::{AddressService, CreateAddressRequest, UpdateAddressRequest};
...
pub async fn request_magic_link(
    ...
    match magic_link_service.create(&req.email).await {
        Ok(token) => {
            ...
            let base_url = dotenvy::var("BLURP__APP__BASE_URL")
                .unwrap_or_else(|_| "https://sorastore.com".to_string());
...
pub async fn register(
    ...
    if let Some(ref email_svc) = state.email {
        ...
        let data = blurp_notification::models::WelcomeData { name };
...
pub async fn forgot_password(
    ...
    if let Ok(Some((user_id, _email))) = user {
        ...
        if let Ok(token) = pwd_svc.create_reset_token(user_id).await {
            ...
            if let Some(ref email_svc) = state.email {
                let data = blurp_notification::models::PasswordResetData {
                    name: req.email.split('@').next().unwrap_or("User").to_string(),
                    reset_link,
...
#[utoipa::path(
    ...
    tag = "auth",
    responses(
        (status = 200, description = "List of addresses"),
        (status = 401, description = "Unauthorized")
    ),
    security(("bearer" = []))
)]
pub async fn list_addresses(
    ...
    let svc = AddressService::new(state.db.clone());
    match svc.list(user_id).await {
        Ok(addrs) => ApiResponse::success(addrs).into_response(),
        Err(e) => e.into_response(),
    }
}
...
pub async fn create_address(
    ...
    let svc = AddressService::new(state.db.clone());
    match svc.create(user_id, req).await {
        Ok(addr) => WithStatus(StatusCode::CREATED, ApiResponse::success(addr)).into_response(),
        Err(e) => e.into_response(),
    }
}
...
#[utoipa::path(
    ...
    tag = "auth",
    params(
        ("id" = String, Path, description = "Address ID")
    ),
    responses(
        (status = 200, description = "Address details"),
...
pub async fn get_address(
    ...
    let svc = AddressService::new(state.db.clone());
    match svc.get(user_id, id).await {
        Ok(addr) => ApiResponse::success(addr).into_response(),
        Err(e) => e.into_response(),
    }
}
...
pub async fn update_address(
    ...
    let svc = AddressService::new(state.db.clone());
    match svc.update(user_id, id, req).await {
        Ok(addr) => ApiResponse::success(addr).into_response(),
        Err(e) => e.into_response(),
    }
}
...
pub async fn delete_address(
    ...
    let svc = AddressService::new(state.db.clone());
    match svc.delete(user_id, id).await {
        Ok(()) => ApiResponse::success(serde_json::json!({"message": "Address deleted"})).into_response(),
        Err(e) => e.into_response(),
    }
}
...
#[utoipa::path(
    ...
    responses(
        (status = 200, description = "Default address set"),
        (status = 404, description = "Address not found")
    ),
    security(("bearer" = []))
)]
pub async fn set_default_address(
    ...
    let svc = AddressService::new(state.db.clone());
    match svc.set_default(user_id, id).await {
        Ok(addr) => ApiResponse::success(addr).into_response(),
        Err(e) => e.into_response(),
    }
}
